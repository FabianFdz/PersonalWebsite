...
2. Run `npm run harness:validate`. Do not continue from invalid control-plane state.
3. Run `npm run harness -- sprint`.
...
4. Run `npm run harness:validate -- <handoff.json>` and then `npm run harness -- ingest <handoff.json>`.
...
- `npm run harness -- sprint` records the active epic as completed.

Before reporting progress or completion, run `npm run harness:validate` once more. Summarize the epic, current phase, last validated artifact, and next required actor.
